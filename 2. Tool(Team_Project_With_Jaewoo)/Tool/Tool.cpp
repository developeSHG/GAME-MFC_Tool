...
BOOL CToolApp::InitInstance()
{
	...
	CRenderMgr::Get_Instance()->Initialize();
	CMyTimeMgr::Get_Instance()->InitTime();
	// OLE 라이브러리를 초기화합니다.
	if (!AfxOleInit())
	{
...
BOOL CToolApp::OnIdle(LONG lCount)
{
	...
	int a = 5;
	BOOL temp = CWinApp::OnIdle(lCount);
	...
	{
		//CMyTimeMgr::Get_Instance()->UpdateTime();
		//CRenderMgr::Get_Instance()->Render();
	}
	else
	{
		CMyTimeMgr::Get_Instance()->UpdateTime();
